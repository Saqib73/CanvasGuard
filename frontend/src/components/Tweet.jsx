    ...
    <div className="flex gap-3 p-4 border-b border-neutral-300 dark:border-neutral-800 hover:bg-neutral-50 dark:hover:bg-black/5 transition-colors duration-200">
      ...
      <div className="flex-1">
        <div className="flex gap-2 items-center text-sm">
          <span className="font-semibold text-cg-text">{author?.name}</span>
          {author?.verified && <span className="text-sky-400">✔</span>}
          <span className="text-neutral-500 dark:text-neutral-400">@{author?.handle}</span>
        </div>
        <Link to={`/post/${post.id}`} className="block whitespace-pre-wrap text-[15px] leading-5 mt-1 hover:underline text-cg-text">
          {post.text}
        </Link>
        {post.mediaUrl && (
          <Link to={`/post/${post.id}`} className="mt-2 block overflow-hidden rounded-2xl border border-neutral-300 dark:border-neutral-800 shadow-lg shadow-black/30">
        ...
        <div className="flex justify-between text-neutral-500 dark:text-neutral-400 text-sm mt-3 max-w-md">
          <button className={`hover:text-sky-400 flex items-center gap-2 transition-colors duration-200`} onClick={() => { }}>
            <span>💬</span>
            <span>Reply</span>
          </button>
          <button className={`${isReposted ? 'text-green-500' : 'hover:text-green-500'} flex items-center gap-2 transition-colors duration-200`} onClick={() => toggleRepost(post.id, 'me')}>
            <span>🔁</span>
            <span>Repost {post.reposts.size ? formatCount(post.reposts.size) : ''}</span>
          </button>
          <button className={`${isLiked ? 'text-pink-500' : 'hover:text-pink-500'} flex items-center gap-2 transition-colors duration-200`} onClick={() => toggleLike(post.id, 'me')}>
            <span>❤</span>
            <span>Like {post.likes.size ? formatCount(post.likes.size) : ''}</span>
          </button>
          <button className="hover:text-neutral-700 dark:hover:text-neutral-200 flex items-center gap-2 transition-colors duration-200" onClick={() => { }}>
            <span>📤</span>
            <span>Share</span>
          </button>
        </div>
