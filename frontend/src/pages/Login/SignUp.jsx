...
export default function SignupForm({ setIsLogin }) {
  ...
  const handleSignup = async (e) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        toast.success(
          data.message || "SignUp successfull! OTP sent to your mail",
          { id: toastId }
        );
      } else {
  ...
  const handleOtpVerify = async (e) => {
    e.preventDefault();
    console.log(userId, form.otp);
    try {
      const { data } = await axios.post(
        `${import.meta.env.VITE_SERVER}/api/v1/auth/verify`,
        { userId, otp: form.otp },
        {
          withCredentials: true,
        }
  ...
  return (
    <div className="flex min-h-full flex-col justify-center px-4 py-4 lg:px-8 overflow-x-scroll">
      ...
      <div className="mt-10 sm:mx-auto sm:w-full sm:max-w-sm">
        <form
          className="space-y-6"
          onSubmit={!otpSent ? handleSignup : handleOtpVerify}
        >
          {!otpSent ? (
            <>
